...
// we are going to create remote proxy which is going to be a cache of objects before accessing the DB.
// we have DB with many users but instead of accessing the DB each time we want info about user we will have FIFO stack of users in proxy pattern

type User struct {
	ID int
}

//interface that database and proxy implements
...
// ----------------------------------------------------------------------
// will cache maximum of StackSize users
